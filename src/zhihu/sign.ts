...
// Reverse-engineered Zhihu x-zse-96 signing.
// Derived from public research: iteng007/zhihu-mcp-server and zly2006/zhihu-plus-plus.
...
function encryptBlock(input: Uint8Array): Buffer {
...
function encryptBlocks(data: Uint8Array, initialVector: Uint8Array): Buffer {
  let vector: Buffer = Buffer.from(initialVector);
...
export function encryptZseV4(input: string): string {
  ...
  if (plainBytes.length > 16) {
    encryptBlocks(plainBytes.subarray(16), firstCipher).copy(cipher, 16);
  }
